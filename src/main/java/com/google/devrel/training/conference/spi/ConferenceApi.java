...
public class ConferenceApi {
	 ...
	 * 
	 * 
	 *            this method, null when the user is not signed in.
	...
	public WrappedBoolean registerForConference(final User user,
		...
		WrappedBoolean result = ofy().transact(new Work<WrappedBoolean>() {
			public WrappedBoolean run() {
				try {
					...
					Key.create(websafeConferenceKey);
					...
					return new WrappedBoolean(false, "Unknown exception: " + e.getMessage());
				}
			}
		});
		// if result is false
		if (!result.getResult()) {
			...
			if (result.getReason() == "No seats available") {
	...
	public Collection<Conference> getConferencesToAttend(final User user)
		...
		Collection<Conference> coll = new ArrayList<Conference>(0);
		for (String k : keyStringsToAttend)
			coll.add(getConference(k));
		return coll;
	}
	...
	public WrappedBoolean unregisterFromConference(final User user,
			@Named("websafeConferenceKey") final String websafeConferenceKey) throws UnauthorizedException, NotFoundException, ForbiddenException {
		// If not signed in, throw a 401 error.
		if (user == null) {
			throw new UnauthorizedException("Authorization required");
		}
		...
		WrappedBoolean result = ofy().transact(new Work<WrappedBoolean>() {
			public WrappedBoolean run() {
				try {
					...
					if (profile.getConferenceKeysToAttend().contains(websafeConferenceKey)) {
						/*
						 * return new WrappedBoolean (false, "Already registered"); } else if
						 * (conference.getSeatsAvailable() <= 0) { return new WrappedBoolean (false,
						 * "No seats available"); } else { // All looks good, go ahead and book the seat
						 */
						// TODO
						// Add the websafeConferenceKey to the profile's
						// conferencesToAttend property
